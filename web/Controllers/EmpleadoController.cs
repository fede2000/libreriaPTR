...
public class EmpleadoController : Controller
{
  ...
  public EmpleadoController(ISecurityServices security, ILogger<EmpleadoController> logger)
  {
    _security = security;
    _logger = logger;
  }
  ...
  {
    var vm = new NewEmpleadoViewModel
    {
      Usuario = new Usuario(),
  ...
  [HttpPost]
  public IActionResult Nuevo([Bind(Prefix = "Empleado")]Usuario user, string hashedPass, int[] perfilesAsignados)
  {
    ...
    {
      ModelState.AddModelError<NewEmpleadoViewModel>(vm => vm.Usuario.Login, "El login no puede estar vacio");
    }
    if (user.Nacimiento.Year <= 1900)
    {
      ModelState.AddModelError<NewEmpleadoViewModel>(vm => vm.Usuario.Nacimiento,
